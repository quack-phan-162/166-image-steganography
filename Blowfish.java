...
//                        + right + left);
    ...
    String ASCIItoHex(String ASCII){
        ...
        String binary = "";
        for (int i = 0; i < 8; i++){
            int dec = (int) ASCII.charAt(i);
            binary = Integer.toBinaryString(dec);
            String padding = "";
            int zeroes = 8 - binary.length();
            if (zeroes > 0) {
                for (int x = 0; x < zeroes; x++) padding += "0";
...
//        System.out.println(binary);
        for (int i = 0; i < binary.length(); i+=8){
            ...
            output += Character.toString(decimal);
        }
        return output;
    }
    ...
    Blowfish()
    {
       ...
       // String key = "aabb09182736ccdd";
        ...
        for (int i = 0; i < (message.length()/8); i++){
            String input = message.substring(start, start+8);
            plainText = ASCIItoHex(input);
            cipherMessage += encrypt(plainText);
            start+=8;
        }
        long endEncrypt = System.currentTimeMillis();
        System.out.println("Cipher Text: " + cipherMessage);

        long encryptionTime = endEncrypt-startEncrypt;
        ...
        start = 0;
        for (int i = 0; i < (cipherMessage.length()/16); i++){
            String output = cipherMessage.substring(start, start+16);
            String decipherText = decrypt(output);
            decipherMessage += HexToASCII(decipherText);
            start+= 16;
        }
        long endDecrypt = System.currentTimeMillis();
        System.out.println("Decipher Text: " + decipherMessage);
        long decryptionTime = endDecrypt - startDecrypt;
